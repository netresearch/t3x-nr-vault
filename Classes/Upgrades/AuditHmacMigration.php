...
/**
 * Migrates the audit log hash chain to the configured HMAC epoch.
 *
 ...
 *
 * This class holds the logic and deliberately implements no TYPO3 upgrade
 * interface: TYPO3 13 and 14 declare that interface in different packages.
 * The Install Tool reaches it through {@see AuditHmacMigrationWizard}
 * (TYPO3 14) or {@see AuditHmacMigrationWizardV13} (TYPO3 13), whichever
 * `Configuration/Services.php` registers for the running major.
 *
 ...
 */
final class AuditHmacMigration implements LoggerAwareInterface
{
    use AuditChainLockTrait;
    use LoggerAwareTrait;

    /**
     * The identifier TYPO3 records the wizard under in `sys_registry` once it
     * has run. Changing it would make every installation run it again.
     */
    public const IDENTIFIER = 'nrVaultAuditHmacMigration';
    ...
    public function executeUpdate(): bool
    {
        ...
        if ($failedVerification instanceof HashChainVerificationResult) {
            $this->logger?->error(
                'AuditHmacMigration:existing audit chain failed verification; refusing to re-seal',
        ...
        try {
            ...
            $this->logger?->error(
                'AuditHmacMigration:re-hash failed; transaction rolled back',
                ['exception' => $e],
            );
    ...
    private function rehashChain(Connection $connection, string $hmacKey, int $targetEpoch): bool
    {
        ...
        try {
            ...
            $this->logger?->info(
                'AuditHmacMigration:migrated audit chain to HMAC',
