...
- **CI's Rector job resolves the newest Rector release, so a new rule can turn `main` red without a commit.** Rector 2.6.7 added `DirnameDirConcatStringToDirectStringPathRector` and failed `main` (`6fa9b9c`) and every open PR over the two test bootstraps. When the log names files your change did not touch, check `main`'s latest CI run first. A local `.Build` usually lags the release: to confirm a fix, `composer require rector/rector:<version>` into a scratch directory and run `rector process <files> --dry-run` with a config holding only that rule (`RectorConfig::configure()->withRules([...])`) — the repo's `rector.php` needs the TYPO3 Rector sets and will not load there. Old code reports "would have been changed", fixed code "Rector is done!".
